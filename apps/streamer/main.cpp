...
const string defaultH264SamplesDirectory = defaultRootDirectory + "h2644/";
...
shared_ptr<ClientTrackData> addVideo(const shared_ptr<PeerConnection> pc, const uint8_t payloadType, const uint32_t ssrc, const string cname, const string msid, const function<void (void)> onOpen) {
    ...
    auto rtpConfig = make_shared<RtpPacketizationConfig>(ssrc, cname, 96, H264RtpPacketizer::defaultClockRate);
    // create packetizer
    ...
    auto root = make_shared<MediaHandlerRootElement>();
    auto mediaHandler = make_shared<MediaChainableHandler>(root);
    // add RTCP SR handler
...
shared_ptr<Client> createPeerConnection(const Configuration &config,
    ...
    client->video = addVideo(pc, 96, 42, "video-stream", "stream1", [id, wc = make_weak_ptr(client)]() {
    ...
    stream->onSample([ws = make_weak_ptr(stream)](Stream::StreamSourceType type, uint64_t sampleTime, rtc::binary sample) {
        ...
        if (!tracks.empty()) {
            for (auto clientTrack: tracks) {
                ...
                //cout << "Sending " << streamType << " sample with size: " << to_string(sample.size()) << "in " << " to " << client << endl;
                bool send = false;
...
/*shared_ptr<Stream> createStream_overRTP(const unsigned fps) {
    cout << "Creating Stream" << std::endl;
